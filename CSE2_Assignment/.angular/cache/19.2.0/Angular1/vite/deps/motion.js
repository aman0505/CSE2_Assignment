...
function spring(optionsOrVisualDuration = springDefaults.visualDuration, bounce = springDefaults.bounce) {
  ...
  } = getSpringOptions(__spreadProps(__spreadValues({}, options), {
...
function createGeneratorEasing(options, scale2 = 100, createGenerator) {
  const generator = createGenerator(__spreadProps(__spreadValues({}, options), {
    keyframes: [0, scale2]
  }));
...
var MAX_REPEAT = 20;
function createAnimationsFromSequence(sequence, _a = {}, scope, generators2) {
  var _b = _a, {
    defaultTransition = {}
  } = _b, sequenceTransition = __objRest(_b, [
    "defaultTransition"
  ]);
  ...
  for (let i = 0; i < sequence.length; i++) {
    ...
    const resolveValueSequence = (valueKeyframes, valueTransition, valueSequence, elementIndex = 0, numSubjects = 0) => {
      ...
      const _a2 = valueTransition, {
        ...
        repeatType,
        repeatDelay = 0
      } = _a2, remainingTransition = __objRest(_a2, [
        "delay",
        "times",
        "type",
        "repeat",
...
var MainThreadAnimation = class extends BaseAnimation {
  ...
  initPlayback(keyframes$1) {
    ...
    const generator = generatorFactory(__spreadProps(__spreadValues({}, this.options), {
...
var AcceleratedAnimation = class extends BaseAnimation {
  ...
  initPlayback(keyframes2, finalKeyframe) {
    ...
    const animation = startWaapiAnimation(motionValue2.owner.current, name, keyframes2, __spreadProps(__spreadValues({}, this.options), {
      duration,
      times,
      ease: ease2
    }));
...
function animateTarget(visualElement, targetAndTransition, {
  ...
  var _a;
  let _a2 = targetAndTransition, {
    transition = visualElement.getDefaultTransition(),
    transitionEnd
  } = _a2, target = __objRest(_a2, [
    "transition",
    "transitionEnd"
  ]);
...
var VisualElement = class {
  ...
  constructor({
    ...
    const _a = this.scrapeMotionValuesFromProps(props, {}, this), {
      willChange
    } = _a, initialMotionValues = __objRest(_a, [
      "willChange"
    ]);
...
function buildSVGAttrs(state2, _a, isSVGTag2, transformTemplate) {
  var _b = _a, {
    attrX,
    attrY,
    attrScale,
...
function transformBoxPoints(point2, transformPoint) {
  if (!transformPoint) return point2;
...
function animateSubject(subject, keyframes2, options, scope) {
  ...
  if (isSingleValue(subject, keyframes2)) {
    ...
    for (let i = 0; i < numSubjects; i++) {
      ...
      animations.push(...animateTarget(visualElement, __spreadProps(__spreadValues({}, keyframes2), {
        transition
      }), {}));
    }
  }
...
function animateElements(elementOrSelector, keyframes2, options, scope) {
  ...
  for (let i = 0; i < numElements; i++) {
    ...
    for (const valueName in keyframes2) {
      ...
      const valueOptions = __spreadValues({}, getValueTransition2(elementTransition, valueName));
